...
| 12 | **Repudiation** | No audit trail for auth events | `events.go:64-84` | `emitEvent()` persists security events (`login.success/failure`, `registration.*`, `session.revoke`, `password.change`, `challenge.*`, `rate_limit.reject`, etc.) to the `security_event` table; events older than 90 days pruned on startup and daily | Fire-and-forget semantics — a failed write logs the error but never blocks auth |
