...
func TestRevokeSessions_DBError(t *testing.T) {
	sessionTestSetup(t)
	store.Close() // force DB errors

	_, code, _ := revokeSessions("all", nil)
	if code != "INTERNAL_ERROR" {
		t.Errorf("revokeSessions on closed DB: code = %q, want %q", code, "INTERNAL_ERROR")
	}

	// Reinit so cleanup doesn't panic
	initDB(":memory:")
}
